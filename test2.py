...
for i in range(X.shape[0]):
    for j in range(X.shape[0]):
        Hessian[i][j] = y[i] * y[j] * (np.dot(X[i], X[j]) + lambda_val**2)

# Proses pelatihan SVM menggunakan metode sequential minimal optimization (SMO)
...
for i in range(X_test.shape[0]):
    f_x = np.dot(a * y, np.dot(X, X_test[i])) + b
    if f_x > 0:
